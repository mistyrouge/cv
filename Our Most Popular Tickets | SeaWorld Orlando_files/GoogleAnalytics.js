...
// Version 9 2012/03/29
// Updates:
//		2010/08/26
// 	- Alow for max sessions.  This is to keep the utmv cookie from getting too larts.
// 	- Trim utmv cookie if there a more sessions than maxSessions. 
//		  This fixes users that went over the limit before the limit was put in place.
//		- Check that current session is different from most recent session before appending.
//		2010/11/30
//		- Forked attr.js code to create this file (attr_as.js) which is used for attribution with asynch code.
//		2011/11/13
//		- Updated addSource() function and removed creation of pageTracker object.
//		- Updated call to appendV() so that pageTracker object was not an arguement.  This was not being used any longer but still part of the code.
//		- Updated addSource() to accept an optional 2nd argument that is the name of the asych tracker object.  This was requried because the code
//		  assumes there is an unnamed asych tracker, but this may not always be the case.
var seaQP = new Object();
seaQP["google"]         = "q";
seaQP["yahoo"]          = "p";
seaQP["msn"]            = "q";
seaQP["aol"]            = "query";
...
seaQP["onet"]           = "qt";
seaQP["netsprint"]      = "q";
seaQP["google.interia"] =  "q";
seaQP["szukacz"]        = "q";
seaQP["yam"]            = "k";
seaQP["pchome"]         = "q";
seaQP["kvasir"]         = "searchExpr";
seaQP["sesam"]          = "q";
seaQP["ozu"]            = "q";
seaQP["terra"]          = "query";
seaQP["nostrum"]        = "query";
seaQP["mynet"]          = "q";
seaQP["ekolay"]         = "q";
seaQP["search.ilse"]    = "search_for";
seaQP["bing"]           = "q";

 // No need to get the value of utmb so that we can compare with utmb_iq.  The new method is to look for the existence of utmb.
 // obj._initData();

 // No need to compare utmb_iq and utmb, The new method is just to look for the existence of utmb.
 // var utmb_iq  = _uGCn(document.cookie, '__utmb_iq=', ';');
function addSource(_gaq,tN){

	// Test for existence else set as empty string.
	tN = (typeof tN == "undefined")?'':tN;

	var utmb   = _uGCn(document.cookie, '__utmb=', ';');
	var iq_data_q  = readIQDataQuery();
	var ga_data_q  = readGADataQuery();
	var v;

	if(iq_data_q['source'] != '-'){
		_gaqiq.push(['_initData']);

		var iq_data_c = readIQDataCookie();
		iq_data_q['search_term']   = _uGCn(getReferrerQuery(), seaQP[iq_data_c[ 'source' ] ]+'=', '&');
		// PopupData('iq query data',iq_data_q);
		v = appendV(iq_data_q);

		var tmpDom = document.location.host.toString();
		delCookie('__utmiqa',tmpDom);
		delCookie('__utmiqz',tmpDom);
		delCookie('__utmiqb',tmpDom);
		delCookie('__utmiqc',tmpDom);
	}else if(ga_data_q['source'] != '-'){
		_gaqiq.push(['_initData']);

		var iq_data_c = readIQDataCookie();
		ga_data_q['search_term']   = _uGCn(getReferrerQuery(), seaQP[iq_data_c[ 'source' ] ]+'=', '&');

		// PopupData('ga query data',ga_data_q);
		v = appendV(ga_data_q);
		...
		var iq_data_c = readIQDataCookie();
		iq_data_c['search_term']   = _uGCn(getReferrerQuery(), seaQP[iq_data_c[ 'source' ] ]+'=', '&');
		// PopupData('iq cookie data',iq_data_c);
		v = appendV(iq_data_c);
		...
		// Do nothing.  This session has already been processed.
	}

	if(v){
		_gaq.push([tN + '_setVar',v]);
	}
}


function setCookie(name,value,domain,path,days) {
 var cValue, cDomain, cExpires, cPath;
 ...
 document.cookie = cValue+cExpires+cDomain+cPath;
}


function delCookie(name,domain) {
 domain = domain.replace(/^www\./,'');
 setCookie(name,"",domain,'/',-2);
}

function appendV(data){

 // Session Delimiter
 var sDelimiter = ' -> ';

 // Max sessions to save
 var maxSessions = 15;
 
 // Current value of _utmv
 v = unescape(_uGCn(document.cookie, '__utmv=', ';'));

 // strip custom var data from __utmv cookie, if they are present.
 if ( utmvHasCustomVars(v) ){
  v = v.substring(0, v.lastIndexOf('|'));
 }

 // Clean empty sessions from history
 v =  v.replace(/\|\|\|-\|-\|- -> /g,'');
 v =  v.replace(/\|\|\|-\|-\|-/g,'');
 v =  v.replace(/ -> $/,'');

 // Get rid of userId at beginning of v.
 v = v.substring(v.indexOf('.') +1);

 // Stop recording sessions at maxSessions and trim sessions if there are more.
 var savedSessions = countSavedSessions(v,sDelimiter);
 if(savedSessions > maxSessions){
   v = trimV(v,sDelimiter,maxSessions); 
   return v;
 }else if(savedSessions == maxSessions){
   return false;
 }

 // Clean up search_term
 data['search_term']  = unescape( data['search_term'] );
 data['search_term']  = data['search_term'].replace(/\+/g,' ');

 data['term']    = unescape( data['term'] );
 data['term']    = data['term'].replace(/\+/g,' ');

 // If medium is organic then set campaign to empty.  This is agree with how the GA UI displays this data.
 ...
 var newData = dateStr+'|'+data['source']+'|'+data['medium']+'|'+data['campaign']+'|'+data['content']+'|'+data['term']+'|'+data['search_term'];
 var latestV = getLatestV(v,sDelimiter);

 // IF new session is the same as old session then do not append.	
 if(newData == latestV){
   return false;
 }
 ...
 if (v == '-'){
  ...
  // v = v.substring(v.indexOf('.') +1) + sDelimiter + newData;
  v = v + sDelimiter + newData;
 }
 return v; 
}


function utmvHasCustomVars(v){
  // Get async Tracker.
 ...
 // If we got this far, then no custom vars found
 return false;
}

function readIQDataQuery(){
 var z = window.location.search;
 var returnObj = new Object();

 returnObj['gclid']       = _uGCn(z, 'gclid=',   '&');
 returnObj['source']      = _uGCn(z, 'iq_source=',  '&');
 returnObj['medium']      = _uGCn(z, 'iq_medium=',  '&');
 returnObj['term']      = _uGCn(z, 'iq_term=',   '&');
 returnObj['campaign']    = _uGCn(z, 'iq_campaign=', '&');
 returnObj['content']     = _uGCn(z, 'iq_content=',  '&');


 return returnObj;
}

function readIQDataCookie(){
 var z = _uGCn(document.cookie, '__utmiqz=', ';');
 var returnObj = new Object();

 returnObj['source']      = _uGCn(z, 'utmcsr=',  '|');
 returnObj['medium']      = _uGCn(z, 'utmcmd=',  '|');
 returnObj['term']        = _uGCn(z, 'utmctr=',  '|');
 returnObj['content']     = _uGCn(z, 'utmcct=',  '|');
 returnObj['campaign']    = _uGCn(z, 'utmccn=',  '|');
 returnObj['gclid']       = _uGCn(z, 'utmgclid=', '|');

 if ( returnObj['gclid'] != '-' ){
  returnObj['source']     = 'google';
  returnObj['medium']     = 'cpc';
  // returnObj['campaign']   = 'unknown';
  // returnObj['content']   = 'unknown';
 }

 return returnObj;
}

function readGADataQuery(){
 var z = window.location.search;
 var returnObj = new Object();

 returnObj['gclid']       = _uGCn(z, 'gclid=',    '&');
 returnObj['source']      = _uGCn(z, 'utm_source=',   '&');
 returnObj['medium']      = _uGCn(z, 'utm_medium=',   '&');
 returnObj['term']      = _uGCn(z, 'utm_term=',   '&');
 returnObj['campaign']    = _uGCn(z, 'utm_campaign=',  '&');
 returnObj['content']     = _uGCn(z, 'utm_content=',  '&');

 if (returnObj['gclid'] != '-'){
  returnObj['source']   = 'google';
  returnObj['medium']   = 'cpc';
  // returnObj['campaign']  = 'unknown';
  // returnObj['term']   = 'unknown';
 }
...
function _uGCn(l,n,s) {
 ...
 if (i > -1) {
  ...
  c=l.substring((i+i3),i2);
   ...
   // If c is all whitespace or empty then set it to '-'
   c = c.replace(/^\s+|\s+$/g, '');
   if (c == ''){
      c='-';
   }
...
function PopupData(type,obj){
 var msg = '';
 
 msg += 'data type: '+ type+"\n";
 msg += 'gclid: '+ obj['gclid']+"\n";
 msg += 'source: '+obj['source']+"\n";
 msg += 'medium: '+obj['medium']+"\n";
 msg += 'term: '+obj['term']+"\n";
 msg += 'campaign: '+obj['campaign']+"\n";
 msg += 'content: '+obj['content']+"\n";
 msg += 'search_term: '+obj['search_term']+"\n";
...
function getDate(){
 ...
 // 0 pad left.
 y = Array(3 - y.length).join('0') + y;
 m = Array(3 - m.length).join('0') + m;
 d = Array(3 - d.length).join('0') + d;

 return y + m + d;
}

// return '-' if there was no latest v (This is their first visit)
function getLatestV(v,delimiter){

	if(v.lastIndexOf(delimiter) <0){
		return v;
	}else{
	 return v.substring(v.lastIndexOf(delimiter) + delimiter.length);
	}
}

function countSavedSessions(v,delimiter){
	if(v == '-'){
		return 0;
	}

	return v.split(delimiter).length;

}

function trimV(v,delimiter,sessions){
	var vparts 	= v.split(delimiter);
	var nvparts 	= Array();
 ...
 *				be tracked in Google Analytics.  It should be included in the head tag or
 * 			as close to the top of the page as possible. 
 *
 * Purpose:
 *    To determines the correct Google Analytics account ID and make all 
 *		appropriate page tracking and e-commerce calls for the various Seaworld web properties. 
 *
 * Updates:
 * 	- Version 4   (2011-04-18)
 * 	- Version 4.1 (2011-06-09) - Added _trackPageLoadTime call.
 * 	- Version 4.2 (2011-07-07) - Proper tracking of Vacation Promotion Landing Page (http://seaworldparks.com/seaworld_vacations).
 * 	- Version 4.3 (2011-08-30) - Purchaser History Tracking via user level custom var.
 * 	- Version 4.4 (2011-10-13) - Automatic inclusion of attribution code.
 * 	- Version 4.5 (2011-11-13) - Inclusion of gaiq.js and attr_as.js 
 * 	- Version 5.0 (2012-04-11) - Removed _trackPageLoadTime - this has been deprecated.  
 *											- Updated _trackLinkClic() to handle the following "link types":
 *													'External Links','Select Product Clicks','Tab Clicks','#add-to-cart','#add-to-cart-and-continue'
 *													and the special case Exernal Link to 4adventure.com
 *											- Added automatic tracking for mobile sites.  This is triggered be the existence of either 
 *													the quiery parameter m=1 or the cookie forcedesktop
 *											- Began tracking the page seaworldparks.com/ to both the rollup and catchall accounts. Before this it was 
 *													only tracked to the catchall.
 * 	- Version 5.1 (2012-04-23) - Added function trackPortalPageview().  This automatically records a virtual page view for the portal referring page
 *											  when a visitor clicks from the portal page to a park specific page. 
 * 	- Version 5.2 (2012-07-16) - Added accounts for christmas town florida and williamsburg and added /christmastown portal page.
 * 										- Disabled mobile tracking by not calling isMobile().
 * 	- Version 5.3 (2012-08-13) - Added accounts for HOS SWT. Updated portal virtual pageview for howl-o-scream.com referrals.
 * 	- Version 5.4 (2012-09-6)  - Updated christmas town paths for tampa and williamsburg.
 * 	- Version 5.5 (2012-09-7)  - New HOS SWT paths. 
 *		- Version 5.6 (2012-09-26) - New portalPage lookups for seaworldcorporate, seaworldchristmas 
 *											  New portalReferral CVs for howloscream.com, buschgardens.com, seaworld.com
 *											  Updated account IDs for christmastown.
 *											  Updated trackPortalPageview so that the referring pagepath is tracked as a virtual pageview only if it came from seawoldparks.com.
 *													It doesn't make sense to thrack referring pagepath of external site.  Instead these are tracked as PortalReferral custom varibales.
 *		- Version 5.7 (2012-09-26) - Upated christmastownfl URL paths to christmastownbgt.
 */

// If a user lands on a park page from one of these domains then clicks over to seaworldparks.com track virtual pageview of referral page.
var portalReferrers 	= ['seaworld.com','buschgardens.com','howloscream.com'];

// If a user views any of the below pages on the seaworldparks.com domain then clicks to a park page, track a virtual pageview to the rollup account.
// These will automatically be compared with and without a preceding '/en' subdirectory. 
var portalPages 	= 	{	'seaworldparks':['','/','/default.aspx'],
								'christmastown':['/christmastown','/christmastown/','/christmastown/default.aspx'],
								'spooktacular' :['/spooktacular','/spooktacular/'],
								'seaworldcorporate' :['/home','/home/'],
								'buschgardens' :['/buschgardens','/buschgardens/'],
								'seaworldchristmas' :['/seaworldchristmas','/seaworldchristmas/']
							};

var primaryAcc 		= false;
var secondaryAcc 		= false;
var tertiaryAcc 		= false;
var rollupAcc 			= false;


// Flags and Variables to be used further down.
var rollupAccType 	= '';
var currentDomain		= getCurrentDomain();
var currentPath		= getCurrentPath();
var referrerObj		= __parseURL(document.referrer);
var portalName			= false;
var isTest				= false;

// Record what items where shown and in what position. This will only be called on the 'Most Popular Tickets' page.
var ticketPositions		= [];
var ticketSubTypes		= [];

var ticketImpressions		= [];
var ancillaryImpressions	= [];

// Set test flag to true if we are viewing a URI like /test/...
//if(currentPath.substring(0,6) == '/test/'){
//	isTest = true;
//}
...
// Default device type. Values are: web|mobile.
var siteType = 'web'; 

/* Mobile tracking is being handled by BGT Partners.
if(isMobile()){
	siteType = 'mobile';
}
*/

// catchAllAcc is to be used if we can not get a primary account.
// This is intened to be used to find traffic that is slipping through the cracks.  
// A properly functioning tracking stragegy would mean this proile remains empty.
var catchAllAcc 			= {'web': 'UA-21344763-2','mobile': 'UA-21344763-4'};

// The master rollup account to which all traffic will be sent.  
var masterRollupAcc 	= {'web':'UA-21344763-1',	'mobile':'UA-21344763-3'};

// Mapping of Path Root to GA Account Ids.  
var Domain2Accs = {
	// Test Entries
	'/test/'								:{'web':{'primary':'UA-26428044-1'} ,'mobile':{'primary':''} },

	// Christmastown sites.
	'/christmastownbgw'					:{'web':{'primary':'UA-6542305-3'},'mobile':{'primary':''} },
	'/en/christmastownbgw'				:{'web':{'primary':'UA-6542305-3'},'mobile':{'primary':''} },
	'/christmastownbgt'					:{'web':{'primary':'UA-6512501-6'},'mobile':{'primary':''} },
	'/en/christmastownbgt'				:{'web':{'primary':'UA-6512501-6'},'mobile':{'primary':''} },

	// SWT Howl-o-scream 
	'/en/seaworld_sanantonio_com/howloscream'	:{'web':{'primary':'UA-6542687-1','secondary': 'UA-6635639-1'} },
	'/seaworld_sanantonio_com/howloscream'		:{'web':{'primary':'UA-6542687-1','secondary': 'UA-6635639-1'} },
	'/seaworld-sanantonio/howloscream'			:{'web':{'primary':'UA-6542687-1','secondary': 'UA-6635639-1'} },
	'/en/seaworld-sanantonio/howloscream'		:{'web':{'primary':'UA-6542687-1','secondary': 'UA-6635639-1'} },

	'/seaworld_vacations'			:{'web':{'primary':'UA-6542653-1','secondary':'UA-6542687-1','tertiary':'UA-6513343-1'}, 'mobile':{'primary':''} },
	'/sea_rescue'						:{'web':{'primary':'UA-6542653-1','secondary':'UA-6542687-1','tertiary':'UA-6513343-1'}, 'mobile':{'primary':''} },
	'/seaworld-sandiego'				:{'web':{'primary':'UA-6542653-1'},'mobile':{'primary':'UA-6542653-4'} },
	'/seaworld_sandiego'				:{'web':{'primary':'UA-6542653-1'},'mobile':{'primary':'UA-6542653-4'} },
	'/sandiego'							:{'web':{'primary':'UA-6542653-1'},'mobile':{'primary':'UA-6542653-4'} },
	'/seaworld-sanantonio'			:{'web':{'primary':'UA-6542687-1'},'mobile':{'primary':'UA-6542687-4'} },
	'/seaworld_sanantonio'			:{'web':{'primary':'UA-6542687-1'},'mobile':{'primary':'UA-6542687-4'} },
	'/sanantonio'						:{'web':{'primary':'UA-6542687-1'},'mobile':{'primary':'UA-6542687-4'} },
	'/seaworld-orlando'				:{'web':{'primary':'UA-6513343-1'},'mobile':{'primary':'UA-6513343-4'} },
	'/seaworld_orlando'				:{'web':{'primary':'UA-6513343-1'},'mobile':{'primary':'UA-6513343-4'} },
	'/orlando'							:{'web':{'primary':'UA-6513343-1'},'mobile':{'primary':'UA-6513343-4'} },
	'/aquatica'							:{'web':{'primary':'UA-5588274-1'},'mobile':{'primary':'UA-5588274-3'} },
	'/buschgardens-tampa'			:{'web':{'primary':'UA-6512501-1'},'mobile':{'primary':'UA-6512501-5'} },
	'/buschgardens_tampa'			:{'web':{'primary':'UA-6512501-1'},'mobile':{'primary':'UA-6512501-5'} },
	'/tampa'								:{'web':{'primary':'UA-6512501-1'},'mobile':{'primary':'UA-6512501-5'} },
	'/adventure-island'				:{'web':{'primary':'UA-6513187-1'},'mobile':{'primary':'UA-6513187-2'} },
	'/adventure_island'				:{'web':{'primary':'UA-6513187-1'},'mobile':{'primary':'UA-6513187-2'} },
	'/buschgardens-williamsburg'	:{'web':{'primary':'UA-6542305-1'},'mobile':{'primary':'UA-6542305-5'} },
	'/buschgardens_williamsburg'	:{'web':{'primary':'UA-6542305-1'},'mobile':{'primary':'UA-6542305-5'} },
	'/williamsburg'					:{'web':{'primary':'UA-6542305-1'},'mobile':{'primary':'UA-6542305-5'} },
	'/water-country-usa'				:{'web':{'primary':'UA-6513234-1'},'mobile':{'primary':'UA-6513234-2'} },
	'/water_country_usa'				:{'web':{'primary':'UA-6513234-1'},'mobile':{'primary':'UA-6513234-2'} },
	'/sesame-place'					:{'web':{'primary':'UA-6513385-1'},'mobile':{'primary':'UA-6513385-2'} },
	'/sesame_place'					:{'web':{'primary':'UA-6513385-1'},'mobile':{'primary':'UA-6513385-2'} },
	'/discovery-cove'					:{'web':{'primary':'UA-6512379-1'},'mobile':{'primary':'UA-6512379-3'} },
	'/discovery_cove'					:{'web':{'primary':'UA-6512379-1'},'mobile':{'primary':'UA-6512379-3'} }
};
...
if(primaryAcc){
	secondaryAcc 	= getAcc('secondary');
	tertiaryAcc 	= getAcc('tertiary');
	rollupAcc 		= masterRollupAcc[siteType];

	// Get portal name, if referrer exists.
	// If portal name !false then a custom var will fire further down.
	if(document.referrer){
		portalName = getPortalName(referrerObj.hostname, referrerObj.path);
	}

}else{
	rollupAcc 		= catchAllAcc[siteType];

	// Get portal name. 
	// If portal name !false then a custom var will fire further down.
	portalName = getPortalName(currentDomain, currentPath);

	// If this is a portal page then set the primary account to be the master account.
	if(portalName){
		primaryAcc = masterRollupAcc[siteType];
	}
}

// Clear out all but primary account if this is a test page.
if(isTest){
	primaryAcc 		= 'UA-26428044-4';
	secondaryAcc 	= false;
	tertiaryAcc 	= false;
	rollupAcc 		= false;
}
...
// Addthis.com configuration
var addthis_config = {};
if(primaryAcc){
	addthis_config = {
      data_ga_property: primaryAcc,
...
// Begin Tracking standard web pages.
if(primaryAcc){
	try{
		...
		// _gaq.push(function(){addSource(_gaq,'')});
		if(portalName){ _gaq.push(['_setCustomVar',4,'PortalView',portalName,2]); }
		trackPortalPageview('');
...
if(secondaryAcc){
	try{
		_gaq.push(['t2._setAccount', secondaryAcc]);
		_gaq.push(['t2._setAllowLinker', true]);
		_gaq.push(['t2._setAllowHash', false]);
		_gaq.push(['t2._setDomainName', currentDomain]);
		// _gaq.push(function(){addSource(_gaq,'t2.')});
		if(portalName){ _gaq.push(['t2._setCustomVar',4,'PortalView',portalName,2]); }
		trackPortalPageview('t2.');
...
if(tertiaryAcc){
	try{
		_gaq.push(['t3._setAccount', tertiaryAcc]);
		_gaq.push(['t3._setAllowLinker', true]);
		_gaq.push(['t3._setAllowHash', false]);
		_gaq.push(['t3._setDomainName', currentDomain]);
		// _gaq.push(function(){addSource(_gaq,'t3.')});
		if(portalName){ _gaq.push(['t3._setCustomVar',4,'PortalView',portalName,2]); }
		trackPortalPageview('t3.');
...
if(rollupAcc){
	try{
		_gaq.push(['tR._setAccount', rollupAcc]);
		_gaq.push(['tR._setAllowLinker', true]);
		_gaq.push(['tR._setAllowHash', false]);
		_gaq.push(['tR._setDomainName', currentDomain]);
		 // _gaq.push(function(){addSource(_gaq,'tR.')});
		if(portalName){ _gaq.push(['tR._setCustomVar',4,'PortalView',portalName,2]); }
...
function custom_addTrans( params ){
	// Purchase History Trakcing.
	_gaq.push(function(){
		var transID = params[0];
		var trackThis = true;	
		var pageTracker = _gat._getTrackerByName('');
		var BH = pageTracker._getVisitorCustomVar(5);
		if(BH==undefined){
			BH=transID;
		}else{ 
			// Check for duplate transID before appending.
			var trackThis = true;
			BHa = BH.split('!');            
			for(i in BHa){
				if(transID == BHa[i]){
		...
		if(trackThis){
			pageTracker._setCustomVar(5,'Purchase_History',BH,1);
			pageTracker._trackPageview('/Purchase_History/' + transID );
			// Track to rollup account also.
			if(rollupAcc){
				var ruTracker = _gat._getTrackerByName('tR');
				ruTracker._trackPageview('/Purchase_History/' + transID );
			}
		}
	});

	if(primaryAcc){
		params.unshift('_addTrans');
		_gaq.push(params);
		params.shift();
	}
	if(secondaryAcc){
		params.unshift('t2._addTrans');
		_gaq.push(params);
		params.shift();
	}
	if(tertiaryAcc){
		params.unshift('t3._addTrans');
		_gaq.push(params);
		params.shift();
	}
	if(rollupAcc){
		params.unshift('tR._addTrans');
		_gaq.push(params);
		params.shift();
	}
}

function custom_addItem( params ){
	if(primaryAcc){
		params.unshift('_addItem');
		_gaq.push(params);
		params.shift();
	}
	if(secondaryAcc){
		params.unshift('t2._addItem');
		_gaq.push(params);
		params.shift();
	}
	if(tertiaryAcc){
		params.unshift('t3._addItem');
		_gaq.push(params);
		params.shift();
	}
	if(rollupAcc){
		params.unshift('tR._addItem');
		_gaq.push(params);
		params.shift();
	}
}

function custom_trackTrans(){
	if(primaryAcc){
...
function custom_push( params ){
	if(primaryAcc){
		_gaq.push(params);
	}

	if(secondaryAcc){
		params[0] = 't2.' + params[0];
		_gaq.push(params);
	}

	if(tertiaryAcc){
		params[0] = 't3.' + params[0];
		_gaq.push(params);
	}

	if(rollupAcc){
		params[0] = 'tR.' + params[0];
		_gaq.push(params);
	}
}

// Save positions and subtypes of all tickets displayed on page.
function TrackP2PLayout(TrackingInfo) {
	for(var i=0; i<TrackingInfo['Products'].length; i++){
		ticketPositions.push(TrackingInfo['Products'][i]['ItemName']);
		ticketSubTypes.push(TrackingInfo['TrackingSubType']);
	}
}

function GetP2PTicketPosition(TrackingInfo) {
   //  console.log(ticketPositions);
	// Get Position of this ticket.
	for(var i=0; i<ticketPositions.length; i++){
		if(ticketPositions[i] == TrackingInfo['ItemName']){
			return i + 1;
		}
	}
	// if we got this far then we didn't find the item.
	return false;
}

function GetP2PTicketSubType(position) {
   // console.log(ticketSubTypes);
	// Conver position to index.
	index = position -1; 
	return ticketSubTypes[index];
}

function TrackP2PClick(TrackingInfo) {
	var path 			= window.location.pathname.toString();
	var pos				= GetP2PTicketPosition(TrackingInfo);
	var ticketsubType	= GetP2PTicketSubType(pos);

	if(TrackingInfo['TrackingType'] != null && TrackingInfo['TrackingType'] != '' ){
		path += '/' + TrackingInfo['TrackingType'];
	}
	if(TrackingInfo['TrackingSubType'] != null && TrackingInfo['TrackingSubType'] != '' ){
		path += '/' + TrackingInfo['TrackingSubType'];
	}
	if(ticketsubType != null && ticketsubType != '' ){
		path += '/' + ticketsubType;
	}
	if(TrackingInfo['ItemName'] != null && TrackingInfo['ItemName'] != '' ){
		path += '/' + TrackingInfo['ItemName'];
	}else if(TrackingInfo['TrackingTitle'] != null && TrackingInfo['TrackingTitle'] != '' ){
		path += '/' + TrackingInfo['TrackingTitle'];
	}
	if(pos){
		path += ' [' + pos +']';
	}

	// alert('Page Path: ' + path);
   _gaq.push( function() { custom_push(['_trackPageview', path]) });
}


function _trackLinkClick(params) {
 	// alert('0 '+ params[0] +'\n' + '1 ' + params[1] + '\n2 ' + params[2] + '\n3 ' + params[3]);
	if(params[1] == 'Select Product Clicks'){
    	_gaq.push(function () { custom_push(['_trackPageview','/button/select/'+params[3] ]) });
	}else if(params[1] == 'Tab Clicks'){
		var tabNum = parseInt(params[0].substring(12));
		tabNum++;
    	_gaq.push(function () { custom_push(['_trackPageview','/tabs/'+params[2]+'/tab'+tabNum]) });
	}else if(params[0] == '#add-to-cart'){
    	_gaq.push(function () { custom_push(['_trackPageview','/button/addToCart']) });
	}else if(params[0] == '#add-to-cart-and-continue'){
    	_gaq.push(function () { custom_push(['_trackPageview','/button/addToCartAndContinue']) });
	}else if( params[0] == 'External Links' ){
		if( params[1].indexOf('4adventure.com') != -1 ){
			var _utmUrl = params[1];
			_gaq.push(function(){
				var gT = _gat._getTrackerByName('tR');
				_utmUrl =  gT._getLinkerUrl(params[1]);
			});
			return _utmUrl;
		}else{
    		_gaq.push(function () { custom_push(['_trackEvent', 'External Link', 'click', params[1], 1, true]) });
			return params[1];
		}
	}else{
    	_gaq.push(function () { custom_push(['_trackEvent', params[1], params[2], params[3], 1, true]) });
	}
}
...
function trackPortalPageview(trackerName){
	if(document.referrer != 'undefined'){
		if(referrerObj.hostname == 'seaworldparks.com'){
			// seaworldparks.com
			var vP = document.referrer.substring(document.referrer.indexOf('/',9) );
			for(var i=0; i<portalPages.length; i++){
				if(vP == portalPages[i]){
					_gaq.push([trackerName+'_trackPageview',vP]);
				}
			}
		}else{
			// seaworld.com, buschgardens.com, howloscream.com
			var Referrer = document.referrer.replace(referrerObj.protocol+'://','');
			Referrer = Referrer.replace('www.','');
			for(var i=0; i< portalReferrers.length; i++){
				if(referrerObj.hostname == portalReferrers[i]){
					_gaq.push([trackerName+'_setCustomVar',4,'PortalReferrer',Referrer,2]);
				}
			}
		}
	}
...
function createCookie(domain,name,value,path) {
        var expires = "";
...
function getAcc(type){
	var cPath 	= window.location.pathname.toString().toLowerCase();
	cPath = cPath.split('/',5).join('/');

	for(var path in Domain2Accs){
		if(cPath.search(path)>=0){
			var data 	= Domain2Accs[path][siteType];
			accountId 		= data[type];
			return accountId;
		}
	}
...
function isMobile(){
	if(getQueryVal('m')){
		return true;
	}else if(getCookieVal('forcedesktop') ){
		return true;
	}else{
		return false;
	}
...
function getPortalName(hostname,path){
	hostname = cleanURL(hostname);
	if(hostname == 'seaworldparks.com'){
		for(var Pn in portalPages){
			portalPaths = portalPages[Pn];
			for(var i=0; i<portalPaths.length; i++){
				if( (path.toLowerCase() == portalPaths[i]) || ( path.toLowerCase() == '/en' + portalPaths[i] ) ){
					return Pn;
				}
			}
		}
...
/*
function getP2PItemPosition(itemName){
	alert('getP2PItemPosition');
	// Check 'Most Popular Tickets'
	for(var i=0; i<ticketImpressions.length; i++){
		if(itemName == ticketImpressions[i]){
			return i+1;
		}
	}

	// Check Ancillary
	for(var i=0; i<ancillaryImpressions.length; i++){
		if(itemName == ancillaryImpressions[i]){
			return i+1;
		}
	}
...
function _trackP2PClick(params) {
	alert('_trackP2PClick');
	var page 		= params.shift();
	var object 		= params.shift();

	var pageToTitle = {
		'ticket widget'		: 'Ticket Widget',
		'most popular'			: 'Most Popular Tickets',
		'all tickets'			: 'All Tickets',
		'most popular extras': 'Most Popular Extras',
		'all extras'			: 'All Extras',
		'top nav'				: 'Top Navigation'
	};

	var objectToTitle = {
		'tickets'				: 'Tickets Tab Click',
		'vacation packages'	: 'Vacation Tab Click',
		'add to cart'			: 'Add To Cart',
		'build your pass'		: 'Build your Pass',
		'checkout intercept' : 'Checkout Intercept',
	...
	var eAction 	= '';
	var eLabel 	= '';
	var eValue 	= parseInt(1);

	if(page == 'ticket widget'){
		// Ticket Widget Interaction
		eCategory = objectToTitle[object];
		eAction = pageToTitle[page];
	}

	if(object == 'promo code'){
		// Promo Code Submit
		eCategory 	= objectToTitle[object];
		eAction 		= pageToTitle[page];
	}else if(object == 'add to cart'){
		// Add Item To Cart 
		eCategory 	= objectToTitle[object];
		eAction 		= pageToTitle[page];
		eLabel		= params[0];
		...
		// Virtual pageview.
		trackPageview = true;
		if( (page == 'most popular') || (page == 'most popular extras') ){
			// Record impression location
			eLabel = eLabel + ' ['+ getP2PItemPosition(eLabel).toString() +']';
		}
		virtualPagePath = '/button/addToCart/' + eLabel;

	}else if(object == 'build your pass'){
		// Build Passport Link
		eCategory 	= objectToTitle[object];
		eAction 		= pageToTitle[page];
		eLabel		= params[0];
	}else if(object == 'checkout intercept'){
		// Checkout Intercept
		eCategory 	= objectToTitle[object];
		eAction 		= pageToTitle[page];
		eLabel		= params[0];
	...
	if(trackEvent){
		if(primaryAcc){
			_gaq.push(['_trackEvent',eCategory,eAction,eLabel,eValue,true]);
		}
		if(secondaryAcc){
			_gaq.push(['t2._trackEvent',eCategory,eAction,eLabel,eValue,true]);
		}
		if(tertiaryAcc){
			_gaq.push(['t3._trackEvent',eCategory,eAction,eLabel,eValue,true]);
		}
		if(rollupAcc){
			_gaq.push(['tR._trackEvent',eCategory,eAction,eLabel,eValue,true]);
		}
	...
	if(trackPageview){
		if(primaryAcc){
			_gaq.push(['_trackPageview',virtualPagePath]);
		}
		if(secondaryAcc){
			_gaq.push(['t2._trackPageview',virtualPagePath]);
		}
		if(tertiaryAcc){
			_gaq.push(['t3._trackPageview',virtualPagePath]);
		}
		if(rollupAcc){
			_gaq.push(['tR._trackPageview',virtualPagePath]);
		}
	}
}


function _trackP2PLayout(params) {
	alert('_trackP2PLayout');
	var page 		= params.shift();
	var product 	= params.shift();

	if(page=='most popular'){ page = 'Most Popular Tickets'; }

	if(product=='tickets'){ 
		product = 'Ticket Impressions'; 

		// Save offerings to global array.
		ticketImpressions = params;
	}else if(product=='ancillary'){ 
		product = 'Ancillary Impressions'; 

		// Save offerings to global array.
		ancillaryImpressions = params;
	}

	if(primaryAcc){
		var position = parseInt('1');
		for(var i=0; i<params.length; i++){
			_gaq.push(['_trackEvent',product,page,params[i]+' ['+position+']',position,true]);
			position++;
		}
	}
}
